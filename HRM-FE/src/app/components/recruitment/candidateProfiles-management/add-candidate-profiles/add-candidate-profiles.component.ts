...
export class AddCandidateProfilesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.requestApiService.getDataCandidate().subscribe((value) => {
      this.id_temp = 'HS' + (parseInt(value[0].ID) + 1);
    });
    this.requestApiService.getDataRecruitment().subscribe((value) => {
      value.forEach((val) => {
        if (val.approval == 1) {
          //console.log(val.position);
          this.position_temp.push(val.position);
        }
      });
    });
  }
