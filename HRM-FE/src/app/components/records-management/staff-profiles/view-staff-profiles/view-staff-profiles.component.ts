    ...
    this.requestApiService.getDataStaffProfileByID(ID).subscribe((value) => {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (this.paginatior.pageIndex > 0) {
          ...
          console.log(pos);
          if (result.data) {
            this.dataSource.data[pos].status = 'thôi việc';
          }
        } else {
          console.log(index);
          if (result.data) {
            this.dataSource.data[index].status = 'thôi việc';
          }
        }
      });
    });
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (this.paginatior.pageIndex > 0) {
          const pos =
            this.paginatior.pageSize * this.paginatior.pageIndex + index;
          console.log(pos);
          this.dataSource.data[pos].phoneNumber = result.data[0];
          this.dataSource.data[pos].email = result.data[1];
          this.dataSource.data[pos].position = result.data[2];
          this.dataSource.data[pos].department = result.data[3];
          this.dataSource.data[pos].salary = result.data[4];
        } else {
          console.log(index);
          this.dataSource.data[index].phoneNumber = result.data[0];
          this.dataSource.data[index].email = result.data[1];
          this.dataSource.data[index].position = result.data[2];
          this.dataSource.data[index].department = result.data[3];
          this.dataSource.data[index].salary = result.data[4];
        }
      });
    });
  }
...
export class DialogCancelContract {
  ...
  ngOnInit(): void {
    //Called after the constructor, initializing input properties, and the first call to ngOnChanges.
    //Add 'implements OnInit' to the class.
    this.requestApiService.getDataCC().subscribe((value) => {
      console.log(value);
      this.id_temp = 'QD' + (parseInt(value[0].ID) + 1);
    });
  }
  onSubmit(
    id_decision: string,
...
export class DialogDetailStaffProfile {
  ...
  onSubmit(
    ...
    console.log(toJson);
    this.requestApiService.updateDataStaffProfile(formData).subscribe(
      (value) => {
        console.log(value);
        // this.staffProfiles = value;
        if (value) {
          this.dialogRef.close({
            data: [phoneNumber, email, position, department, salary],
          });
        }
      },
      (err) => {
