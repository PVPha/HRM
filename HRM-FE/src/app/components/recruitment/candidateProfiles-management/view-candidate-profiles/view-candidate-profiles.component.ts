    ...
    this.requestApiService.getDataCandidateByID(ID).subscribe((value) => {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (this.paginatior.pageIndex > 0) {
          ...
          this.dataSource.data[pos].status = 'phỏng vấn';

          if (result.data) {
          }
        } else {
          console.log(index);
          this.dataSource.data[index].status = 'phỏng vấn';
    ...
    this.requestApiService.getDataCandidateByID(ID).subscribe((value) => {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (this.paginatior.pageIndex > 0) {
          ...
          console.log(result.data);
          if (result.data) {
            this.dataSource.data[pos].status = 'tuyển dụng';
          }
        } else {
          console.log(index);
          console.log(result.data);
          if (result.data) {
            this.dataSource.data[index].status = 'tuyển dụng';
          }
        }
      });
    });
...
export class DialogInterviewSchedule {
  ...
  interviewers: Interviewer[] = [
    ...
    { id_staff: '002', position: 'Team leader', name: 'Phạm Văn B' },
  ];
...
export class DialogHiringDecision implements OnInit {
  ...
  ngOnInit() {
    ...
    this.requestApiService.getDataStaffProfile().subscribe((value) => {
      console.log(value);
      this.id_temp = 'NV' + (parseInt(value[0].ID) + 1);
    });
  }
